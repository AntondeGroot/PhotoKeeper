...
describe('PreviewStore', () => {
  ...
  it('stores a preview and retrieves it by asset id and size', async () => {
    await store.put('a1', '2048', new Blob(['image-bytes']));

    expect(await store.get('a1', '2048')).toBeDefined();
  });

  it('keys previews by size: a different size is a different entry', async () => {
  ...
  it('returns undefined for an unknown preview', async () => {
    expect(await store.get('nope', '2048')).toBeUndefined();
  });

  it('evicts previews whose asset id is not kept', async () => {
